...
function SectionBlock({ title, genes, activeQuery, initialOpen = false }) {
          ...
          {filtered.length === 0 ? (
            ...
            <>
              {preview.map((gene) => (
                <span
                  key={gene}
                  className={`rounded-full px-3 py-1 text-xs font-medium ring-1 ${activeQuery && gene.toLowerCase().includes(activeQuery.toLowerCase()) ? "bg-indigo-50 text-indigo-700 ring-indigo-200" : "bg-slate-100 text-slate-700 ring-slate-200"}`}
                >
                  {gene}
                </span>
              ))}
...
export default function GenePanelsCatalog() {
  ...
  const filteredPanels = useMemo(() => {
    ...
    return painels.filter((panel) => {
      ...
      if (!q) return true;
      const inBasic = [panel.nome, panel.categoria, panel.tecnologia, panel.descricao, ...(panel.tags || [])]
        .join(" ")
        .toLowerCase()
        .includes(q);
        ...
        <div className="overflow-hidden rounded-[28px] border border-slate-200/80 bg-white/85 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur">
          ...
          <section className="grid gap-4 px-5 py-5 sm:grid-cols-2 xl:grid-cols-4 sm:px-8 lg:px-10">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white/80 p-4 shadow-sm">
              ...
              <div className="mt-1 text-sm text-slate-500">{query || category !== "Todos" ? "Com filtros ativos" : "Sem filtros ativos"}</div>
            </div>
          </section>
            ...
            <aside className="lg:sticky lg:top-4 lg:self-start">
              <div className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-[0_16px_40px_rgba(15,23,42,0.08)] sm:p-6">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700">{selected.tecnologia}</span>
                  <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700">{selected.versao}</span>
                </div>
                  ...
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {selected.amostras.map((item) => (
                        <span key={item} className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700"><TestTube2 className="mr-1 inline h-3.5 w-3.5" />{item}</span>
                      ))}
                    </div>
                  </div>
                  <div>
                  ...
                  <div className="mt-6 rounded-2xl border border-indigo-100 bg-indigo-50 p-4">
                    ...
                    <div className="mt-3 flex max-h-44 flex-wrap gap-2 overflow-auto pr-1">
                      {matchedGenes.map((gene) => (
                        <span key={gene} className="rounded-full bg-white px-3 py-1 text-xs font-medium text-indigo-700 ring-1 ring-indigo-200">{gene}</span>
                      ))}
                    </div>
                  </div>
                ) : null}
                ...
                <div className="mt-8 space-y-3 border-t border-slate-200 pt-6">
                  ...
                  {Object.entries(selected.secoes).map(([key, genes], idx) => (
                    <SectionBlock key={key} title={prettifySectionKey(key)} genes={genes} activeQuery={query.trim()} initialOpen={idx === 0} />
                  ))}
                </div>
              </div>
            </aside>
