...
function geneMatchesTerms(gene, terms) {
...
function SectionBlock({ title, genes, activeQuery, activeTerms = [], initialOpen = false, category, onInspect }) {
  const [open, setOpen] = useState(initialOpen);
  const filtered = useMemo(() => {
    if (!activeTerms.length) return genes;
    return genes.filter((g) => geneMatchesTerms(g, activeTerms));
  }, [genes, activeTerms]);
      ...
      {open ? (
        <div className="mt-5 flex flex-wrap gap-3.5">
          {filtered.length === 0 ? (
            ...
            <>
              {preview.map((gene) => (
                <GeneBadge key={gene} gene={gene} category={category} highlighted={Boolean(activeTerms.length && geneMatchesTerms(gene, activeTerms))} onInspect={onInspect} />
              ))}
              {hidden > 0 ? (
...
export default function GenePanelsCatalog() {
  ...
  const queryTerms = useMemo(() => parseGeneTerms(query), [query]);
  ...
  const filteredPanels = useMemo(() => {
    return painels.filter((panel) => {
      ...
      const inBasic = textMatchesTerms(searchable, queryTerms);
      const inGenes = panel.genes.some((gene) => geneMatchesTerms(gene, queryTerms));
      return inBasic || inGenes;
    });
  }, [queryTerms, category, structureFilter, tumorTypeFilter, diseaseFilter]);
  ...
  const filteredGenesSelected = useMemo(() => {
    if (!queryTerms.length) return selected.genes;
    return selected.genes.filter((gene) => geneMatchesTerms(gene, queryTerms));
  }, [selected, queryTerms]);

  const matchedGenes = useMemo(() => {
    if (!queryTerms.length) return [];
    return filteredGenesSelected.slice(0, showAllMatches ? 500 : 36);
  }, [filteredGenesSelected, queryTerms, showAllMatches]);
            ...
            <div className="grid gap-3 2xl:grid-cols-[minmax(0,1fr)_220px_240px_260px]">
              <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                ...
                <input
                  ...
                  placeholder="Pesquisar gene(s), painel, categoria ou tag… Ex.: TP53, EGFR; KRAS"
                  className="w-full bg-transparent text-sm outline-none placeholder:text-slate-400"
                />
              </label>
              <div className="col-span-full -mt-1 text-xs text-slate-500">
                Separadores aceites para múltiplos genes: vírgula, ponto e vírgula ou nova linha.
              </div>
              ...
              <div className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-[0_16px_40px_rgba(15,23,42,0.08)] sm:p-6">
                ...
                <div className="mt-8 space-y-3 border-t border-slate-200 pt-6">
                  ...
                  {Object.entries(selected.secoes).map(([key, genes], idx) => (
                    <SectionBlock key={key} title={prettifySectionKey(key)} genes={genes} activeQuery={query.trim()} activeTerms={queryTerms} initialOpen={idx === 0} category={selected.categoria} onInspect={inspectGeneEvidence} />
                  ))}
                </div>
              </div>
            </aside>
